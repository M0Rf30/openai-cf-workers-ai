...
# Test script for Cloudflare Workers AI API - Completions Endpoint
# Tests the http://localhost:8787/v1/completions endpoint
...
run_test() {
  ...
  # Build curl command
  local curl_cmd="curl -X POST \"$WORKER_URL/completions\" -H \"Content-Type: application/json\""
...
validate_completion_response() {
  ...
  validate_json_response "$response_file" \
    ...
    && (grep -q '"text"' "$response_file" || grep -q '"delta"' "$response_file")
}
...
log_info "Starting completions endpoint tests for $WORKER_URL"
echo
...
log_info "=== COMPLETIONS ENDPOINT TESTS ==="
...
run_test "Completion with max_tokens" \
  '{
    "model": "@cf/meta/llama-2-7b-chat-int8",
...
run_test "Completion without prompt (should fail)" \
  '{
    "model": "@cf/meta/llama-2-7b-chat-int8"
}' \
  400 \
...
run_test "Completion with invalid model (should fail)" \
  '{
    "model": "invalid-model",
    "prompt": "Hello"
}' \
...
run_test "Completion without auth (should fail)" \
  '{
    "model": "@cf/meta/llama-2-7b-chat-int8",
    "prompt": "Hello"
}' \
